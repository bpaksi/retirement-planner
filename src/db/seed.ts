...
export function checkSeedStatus() {
  ...
  const existingRules = db.select().from(categorizationRules).all();

  return {
    categoriesSeeded: existingCategories.length > 0,
    rulesSeeded: existingRules.length > 0,
  };
}

/**
 * Run the database seed synchronously (called from app if needed)
 */
export function runSeed() {
  ...
  if (existingRules.length > 0) {
    console.log('📋 Categorization rules already exist, skipping...');
  } else {
    // Insert all rules (default + expanded)
    ...
    for (const rule of allRules) {
      ...
      if (seenPatterns.has(lowerPattern)) continue;
    ...
    console.log(`✅ Seeded ${inserted} categorization rules`);
  }

  console.log('🎉 Seed completed!');
}

// Run seed when executed directly (npx tsx src/db/seed.ts)
const isDirectRun = require.main === module || process.argv[1]?.includes('seed.ts');
if (isDirectRun) {
  runSeed();
}
